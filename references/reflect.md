...
`watch.sh` counts completed tools per worker (successful ones only) and prints `REFLECT`
...
When watch prints `REFLECT`, run the shown `reflect.sh` command once. The inquiry has ten
...
- `ROUTE_CORRECTION`: verify its exact quotes, then send only your correction with `note.sh`.

A direct maintainer correction requires `reflect.sh <run> <label> --trigger maintainer`.
...
`status.sh` reads only the default `<run>/.watch-state`.
