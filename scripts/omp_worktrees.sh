...
case "$ACTION" in
  ...
  --diff)
    ...
    for wt in "$RUN"/worktrees/*/; do
      ...
      name=$(basename "$wt")
      printf '\n=== omp/%s vs %s ===\n' "$name" "$BASE"
      git -C "$REPO" diff --stat "$BASE...omp/$name" 2>/dev/null || echo "(branch missing)"
  ...
  --drift|--rebase)
    ...
    for wt in "$RUN"/worktrees/*/; do
      [ -d "$wt" ] || continue
      name=$(basename "$wt")
      behind=$(git -C "$REPO" rev-list --count "omp/$name..$BASE" 2>/dev/null || echo 0)
...
reg = pathlib.Path(os.environ.get("OMP_REGISTRY_DIR",
      os.environ.get("XDG_RUNTIME_DIR", "/tmp") + "/omp-agents"))
...
LIVE
  ...
  --remove-merged)
    ...
    for wt in "$RUN"/worktrees/*/; do
      ...
      name=$(basename "$wt")
      if [ -n "$(git -C "$wt" status --porcelain)" ]; then
        echo "keep omp/$name: uncommitted changes"; continue
      fi
      if git -C "$REPO" merge-base --is-ancestor "omp/$name" "$ARG" 2>/dev/null; then
        git -C "$REPO" worktree remove "$wt" && git -C "$REPO" branch -d "omp/$name" \
          && echo "removed omp/$name"
      else
        echo "keep omp/$name: not merged into $ARG"
      fi
    done
    ;;
  --remove-all)
    for wt in "$RUN"/worktrees/*/; do
      ...
      name=$(basename "$wt")
      if [ -n "$(git -C "$wt" status --porcelain)" ]; then
        echo "refusing omp/$name: uncommitted changes" >&2; continue
      fi
      git -C "$REPO" worktree remove "$wt" && echo "removed worktree omp/$name"
    done
    ;;
  -h|--help) usage ;;
  *) usage >&2; exit 2 ;;
